...
async function createApp(edgespark) {
  ...
  app.delete("/api/user/account", async (c) => {
    ...
    const userId = edgespark.auth.user.id;
    try {
      await edgespark.db.delete(db_schema_exports.inspections).where(eq(db_schema_exports.inspections.userId, userId));
      await edgespark.db.delete(db_schema_exports.users).where(eq(db_schema_exports.users.id, userId));
  ...
  app.post("/api/checkout", async (c) => {
    ...
    if (freeCredits > 0) {
      ...
      await edgespark.db.update(db_schema_exports.users).set({ freeInspections: freeCredits - 1 }).where(eq(db_schema_exports.users.id, userId));
      console.log(`[CHECKOUT] Used 1 admin credit for user ${userId}. Remaining: ${freeCredits - 1}`);
      if (inspectionId) {
        await edgespark.db.update(db_schema_exports.inspections).set({ payment: JSON.stringify({ paid: true, sessionId, provider: "admin_credit" }) }).where(eq(db_schema_exports.inspections.id, inspectionId));
      }
      return c.json({
        success: true,
        sessionId,
      ...
      if (stripeKey) {
        ...
        try {
          ...
          if (session.payment_status === "paid") {
            const updated = await edgespark.db.update(db_schema_exports.orders).set({
              status: "paid",
              paidAt: Math.floor(Date.now() / 1e3)
            }).where(eq(db_schema_exports.orders.id, order.id)).returning();
            order = updated[0];
            if (order.inspectionId) {
              const inspectionRows = await edgespark.db.select().from(db_schema_exports.inspections).where(eq(db_schema_exports.inspections.id, order.inspectionId));
              if (inspectionRows.length > 0) {
                const currentData = tryParse(inspectionRows[0].paymentData);
                await edgespark.db.update(db_schema_exports.inspections).set({
  ...
  app.post("/api/webhooks/stripe", async (c) => {
    ...
    try {
      ...
      if (event.type === "checkout.session.completed") {
        ...
        if (reportId) {
          await edgespark.db.update(db_schema_exports.orders).set({
            status: "paid",
            paidAt: Math.floor(Date.now() / 1e3)
          }).where(eq(db_schema_exports.orders.providerSessionId, session.id));
          const inspectionRows = await edgespark.db.select().from(db_schema_exports.inspections).where(eq(db_schema_exports.inspections.id, reportId));
          if (inspectionRows.length > 0) {
            const currentData = tryParse(inspectionRows[0].payment);
            await edgespark.db.update(db_schema_exports.inspections).set({
  ...
  app.get("/api/public/admin/users", async (c) => {
    ...
    let rows;
    if (q) {
      rows = await edgespark.db.select().from(db_schema_exports.users);
      rows = rows.filter(
    ...
    const enriched = await Promise.all(rows.map(async (u) => {
      const inspectionCount = await edgespark.db.select().from(db_schema_exports.inspections).where(eq(db_schema_exports.inspections.userId, u.id));
      return {
        id: u.id,
        email: u.email,
        name: u.name,
  ...
  app.post("/api/public/admin/grant-credits", async (c) => {
    ...
    const userRows = await edgespark.db.select().from(db_schema_exports.users).where(eq(db_schema_exports.users.email, email));
    if (userRows.length === 0) return c.json({ error: `No user found with email: ${email}` }, 404);
    const user = userRows[0];
    const newBalance = (user.freeInspections ?? 0) + n;
    await edgespark.db.update(db_schema_exports.users).set({ freeInspections: newBalance }).where(eq(db_schema_exports.users.email, email));
